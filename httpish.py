...
class httpcon:
  ...
  def expect200(self):
    ...
    if ret!=200:
      self.close()
      raise HTTPException(self.txt +" did not return 200 (returned "+ret+")!")
    return ret
...
def POST200(url, dict):
  ...
  if ret != 200:
    raise HTTPException("POST to "+url+" did not return 200 (returned "+ret+")!")
  return ret
def GET200(url):
  ret=GETret(url)
  if ret != 200:
    raise HTTPException("GET to "+url+" did not return 200 (returned "+ret+")!")
  return ret
